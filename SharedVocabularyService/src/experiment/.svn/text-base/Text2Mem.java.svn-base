...
public class Text2Mem {
	...
	public static void main(String [] args){
		...
		String root="/Users/kapila/Documents/repository/experiment1/parsed/";
		Text2Mem tr =new Text2Mem(root);
		List <GR> cul=tr.getAuthorList("hwang");
...
//		}
	...
	public Text2Mem(String dir){
	    
		model=buildModel(dir);
	...
	public List <GR> buildModel(String dir){
		...
		try {
			...
			for (int i=0;i<parsedFiles.length;i++){
				...
				if (file.length()>0 ){
					List <GR> tm=readParsed(dir+"/"+parsedFiles[i],parsedFiles[i].split("-")[0]);
