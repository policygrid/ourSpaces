...
public class Node<T> {
    ...
    public void appendPath(List<T> path) {
    
    	Node<T> currentNode = this;
    	for (int i = 0; i < path.size(); i++) {
    		T element = path.get(i);
    		if (!currentNode.getChildren().contains(element)) {
    			//currentNode.addChild(element);
    			//currentNode = element;
    		} else {
    			currentNode = currentNode.getChildren().get(currentNode.getChildren().indexOf(element));
    		}
    	}
