// Original taken from xilinus.com
// Adapted by PolicyGrid
...
Object.extend(Xilinus.Portal.prototype, {
  ...
  remove: function(widget) {
	...
	this._widgets = this._widgets.reject(function(w) { return w._id == widget._id }); 
    this._columns.each(function(column) { column.immediateDescendants().collect(function(element) { if (element.id == widget._id) Element.remove(element); }); });
  ...
  serialize: function() {
    ...
    this._columns.each(function(column) {   
      var p = column.immediateDescendants().collect(function(element) {
        return column.id + "=" + element.widget.getTitle().innerHTML;
      }).join("&") 
      parameters += p + "&"
    });               
