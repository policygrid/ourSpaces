...
function SimpleTextEditor(id, objectId) {
    ...
    this.getEditorHtml = function() {
        ...
        html += '<td><div class="separator"></div></td>';
        html += '<td class="button"><img src="'+this.path+'images/linkR.gif" width="20" height="20" alt="Insert Link to Resource" title="Insert Link to Resource" onclick="'+this.objectId+'.execCommand(\'createlinkR\')"></td>';
        html += '</tr></table></td></tr>';
    ...
    this.execCommand = function(cmd, value) {
        ...
        } else if (cmd == "createlinkR" && !value) {
            var url = prompt("Enter URL to Resource:", "");
            if (url) {
                this.frame.focus();
                this.frame.document.execCommand("unlink", false, null);
                if (this.browser.ie) this.frame.document.execCommand(cmd, false, "simpletexteditor://simpletexteditor/"+url);
