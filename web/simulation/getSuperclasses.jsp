...
DataProvider dp = ps.getDataProvider();
...
function loadSuperclasses<%=Utility.getLocalName(className) %>(){
	<%
	//Get all subclasses
	Vector<String> subClasses = dp.getSubclasses( className);
	...
	for(int i=0;i<subClasses.size();i++){
		...
		Vector<String> superClasses = dp.getSuperclasses( subClass);
%>
	...
	for(int j=0;j<superClasses.size();j++){
		...
		Vector<String> subClassesTemp = dp.getSubclasses( subClass);
